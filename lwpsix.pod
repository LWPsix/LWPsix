...
The interface supports concurrency through lazy evaluation; the user may send a
request, do something else, and then access the results - reading a line will
block only if the line hasn't arrived yet.
